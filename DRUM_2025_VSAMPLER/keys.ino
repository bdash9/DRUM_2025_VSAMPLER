    ...
    if (trigger_on[f]==1){
      ...
      if (nkey<16){
        ...
        switch (modeZ) {
          ...
          case tPad: // play pads — Ben fixed: single trigger
            if (recording){
              bitWrite(pattern[nkey], sstep, 1);
      ...
      //// 8 function keys
        ...
        if (!shiftR1 && !shifting){
          switch (nkey) {
            ...
            // Mute / Solo / Melodic / Clear / First / Last
            case 20:
              if (modeZ==tMute) {
                modeZ=tSolo;
              } else if (modeZ==tSolo){
            ...
            // PLAY — Ben: replaced uClock with esp_timer
            case 22:
              if (playing) {
                stopSeqTimer();
                sstep                = firstStep;
